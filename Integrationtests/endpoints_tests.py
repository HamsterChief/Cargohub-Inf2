...
def test_data_get_orders_for_client():
    ...
    # POST: Maak order
    post_response = requests.post(f"{BASE_URL}/orders", json=order_1, headers={"API_KEY": API_KEY})
    assert post_response.status_code == 201, f"Order creation failed: {post_response.text}"

    # GET: Haal orders op voor client-ID 1
    get_response = requests.get(f"{BASE_URL}/clients/1/orders", headers={"API_KEY": API_KEY})
    assert get_response.status_code == 200, f"Expected 200, got {get_response.status_code}. Response text: {get_response.text}"

    # JSON-decoding proberen
    try:
        orders = get_response.json()
    except requests.exceptions.JSONDecodeError:
        raise AssertionError(f"Response is not valid JSON: {get_response.text}")

    # Controleer of de juiste order aanwezig is
    assert any(order["id"] == 1 for order in orders), "Order not found for client"

    # Cleanup: Verwijder order
    delete_response = requests.delete(f"{BASE_URL}/orders/1", headers={"API_KEY": API_KEY})
    assert delete_response.status_code == 204, f"Order deletion failed: {delete_response.text}"
